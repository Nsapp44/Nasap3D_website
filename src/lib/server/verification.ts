...
import { hashToken, generateNumericCode } from "./tokens";
import { sendMail } from "./mailer";
import { renderEmailHtml, verificationCodeContentHtml } from "./emailTemplate";
import type { VerificationPurpose } from "@prisma/client";
import { WrongCodeError, NoPendingCodeError, TooManyAttemptsError } from "../api/errors";
...
// typed within a minute or two of receiving it. Resending is only allowed
...
// address, and confirming a password change (see the auth/account API
// routes). `payload` is whatever the confirm step needs once the code
// checks out (new email, new password hash, ...). The code is created
...
export { WrongCodeError, NoPendingCodeError, TooManyAttemptsError };
...
// Signup verification code, sent BEFORE any User row exists (see the
// POST /api/auth/signup route) — payload carries the pending
