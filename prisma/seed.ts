...
import { nextCustomerNo } from "../src/lib/server/counter";
import { hashPassword } from "../src/lib/server/password";

// Runs as its own process in the Docker entrypoint, separate from — and
// before — `node server-entry.mjs`, so that file's own `dotenv/config`
// import never reaches this one: without its own, SEED_ADMIN_EMAIL/PASSWORD
// would read as undefined here, silently falling back to the hardcoded
// default admin credentials instead of the real ones.
//
// sanitizeEnv() strips accidental quotes the same way server-entry.mjs
// does — this file reads SEED_ADMIN_EMAIL/PASSWORD and implicitly
// DATABASE_URL (via `new PrismaClient()` below), both real candidates for
// the same quoting bug.
sanitizeEnv();
const prisma = new PrismaClient();

// Mirrors the color catalogue the admin stock screen edits from. `key`
// matches the configurator's material keys; `label` matches the display
...
// Mirrors the front-end's own quality steps.
const QUALITIES = [
