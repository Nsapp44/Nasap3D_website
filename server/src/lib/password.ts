...
// Same policy already enforced client-side today (Account.dc.html):
// >=8 chars, >=1 uppercase, >=1 special character. Re-checked here because
// the client-side check can be bypassed by anyone calling the API directly.
const SPECIAL_RE = /[!@#$%^&*(),.?":{}|<>_\-+=~`[\]/\\;']/;
