...
// pattern as the admin "reclaim storage after printing" action in
// admin.ts) — only the file on disk is reclaimed. Called right after a cart
// line is removed (see routes/cart.ts); a QuoteJob can in principle be
// referenced by more than one CartItem (no unique constraint), hence the
// "still in cart" check rather than assuming this was the only one.
...
// Periodic sweep (see index.ts) — the backstop for quotes that were
// uploaded/analyzed but never added to any cart at all (so cartCleanup.ts
// never gets a chance to run deleteQuoteJobFileIfOrphaned for them), and for
// the rare case the best-effort sendBeacon discard (routes/quotes.ts
// POST /quotes/:id/discard) never reached the server. deleteQuoteJobFileIfOrphaned
// still re-checks cart membership and order history itself, so a quote
// that's old but legitimately still in an active cart is safely skipped
// here regardless of this sweep's own age threshold.
