...
// a boundary edge (count 1, a hole) or a non-manifold edge (count >2) both
// fail printability, judged as a tolerance (MAX_BAD_EDGE_FRACTION) rather
// than an absolute rule, same reasoning as that file's own comment: real
// STL/OBJ exports (even ones real slicers accept fine) virtually always
// carry a handful of boundary/non-manifold edges from floating-point
// precision or minor CAD-tessellation seams — confirmed live on two real
// customer files (0.007% and 0.26% bad-edge fraction respectively, both
// well under this 1% tolerance) that the *stricter* Manifold-library check
// (manifoldCheck.js) rejects outright with no tolerance at all. A real
// hole/missing wall pushes this fraction far higher, easily into the tens
// of percent — this still catches genuinely broken meshes, just doesn't
// reject real, sliceable files over precision noise.
